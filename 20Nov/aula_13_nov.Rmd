...
```{r carrega}
reclamacoes = read_csv(here("reclamacoes-avaliadas-20181113.csv"))
...
```
```{r normaliza_sentimentos}
normalizados <- BBmisc::normalize(sentimentos$sentimento_op30, method= 'range', range = c(5,1))

sentimentos <- cbind(sentimentos, normalizados)

sentimentos %>% 
    write_csv(here("sentimento.csv"))
```
